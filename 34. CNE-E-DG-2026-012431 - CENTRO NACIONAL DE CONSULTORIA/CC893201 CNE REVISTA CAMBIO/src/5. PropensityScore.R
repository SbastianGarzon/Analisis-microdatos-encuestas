...
library(pacman)
p_load(tidyverse, janitor, haven, survey, srvyr, nonprobsvy, readxl)
...
t_encuestas <- foreign::read.spss("data/CC893201_BASE_POLITICA_REVISTA_CAMBIO.sav",use.value.labels = F,to.data.frame = T) 

encuestas<- t_encuestas |> 
  filter(F1 %in% c(1,2)) |>  ## definitivamente votaria, prob si votaria
            select(REGISTRO,mpio=DANE,GENERO,ESTRATO,
                   REDAD,
                   ZONA,
                   edu=P19, #NIVEL_EDU
                   viv=P17, #TIPO_VIVIENDA
                   activ=P18, #ACTIV_SEMANA_PASADA
                   ss = P20) |>  #REGIMEN_SS  
...
est <- nonprob( 
              selection = ~ region+ tamano  + RESTRATO + SEXO + REDAD + NIVEL_EDU + ACTIV_SEMANA_PASADA + REGIMEN_SS + TIPO_VIVIENDA+ZONA,
              outcome = y ~ region +tamano  + RESTRATO + SEXO + REDAD  + NIVEL_EDU + ACTIV_SEMANA_PASADA + REGIMEN_SS + TIPO_VIVIENDA+ZONA,
              svydesign = dsg,
              data = encuestas,
